...
function updateHelpSheet() {
  ...
  try {
    ...
    const data = [
      ...
      [''],
      ['【STEP 2】カレンダーに予定を入れる'],
      ['集計したい予定のタイトルに [作業] などのキーワードを入れてください。'],
      ['例： [作業] ○○様向けデザイン制作'],
      [''],
      ['【STEP 3】設定をする'],
      ['「設定」シートを開き、案件ごとに集計条件を入力してください。'],
      ['・案件名（空欄にすると集計結果では「-」と表示されます）'],
      ['・集計開始日 / 終了日'],
      ['・カレンダー名（空欄ならデフォルト、複数ならカンマ区切り。右側の一覧からコピーがオススメ！）'],
      ['・集計キーワード（カレンダーのタイトルに含まれる文字。例：[作業]）'],
      ['※複数行入力することで、異なる案件を一括で集計できます。'],
      [''],
      ['【STEP 4】集計を実行する'],
      ...
      ['3. 「このアプリは Google で確認されていません」と出たら、左下の「詳細」をクリックします。'],
      ['4. 下に表示される「実績集計ツール（安全ではないページ）に移動」をクリックします。'],
      ['   ※自分で作成・コピーしたプログラムなので安全です。'],
      ['5. 「アクセスできる情報を選択」画面で、「すべてを選択」にチェック✅を入れます。'],
      ['6. 最後に「許可」をクリックすれば完了です！'],
      [''],
      ['※一度許可すれば、次からは表示されません。']
    ];
    ...
    sheet.getRange(18, 1).setFontWeight('bold').setBackground('#fff2e6');
    sheet.getRange(22, 1).setFontWeight('bold').setBackground('#fdf2f2').setFontColor('#d93025');
